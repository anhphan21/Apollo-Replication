...
bool UserParam::read(int argc, char** argv) { return readNormal(argc, argv, UserParamExtHelper()); }
bool UserParam::readNormal(int argc, char** argv, UserParamExtHelper const& helper) {
  ...
  dreamplaceSPrint(
      kNONE, buf, "%d,%d,%d,%d", defaultParam.drawRegion[0], defaultParam.drawRegion[1], defaultParam.drawRegion[2], defaultParam.drawRegion[3]);
  ...
  desc.add_option(Value<bool>("--help", &help, "print help message").default_value(help).help(true))
      ...
      .add_option(Value<std::string>("--yaml_input", &yamlInput, "input YAML file for PIC circuit"))
      .add_option(Value<std::string>("--def_size_input", &defSizeInput, "input def size file for benchmarks from CUHK"))
      .add_option(Value<std::string>("--def_output", &outputDesign, "output DEF file"))
      ...
      .add_option(
          Value<unsigned>("--sbin_width", &binSize[2 + kX], "sbin width (#rows) in horizontal direction").default_value(defaultParam.binSize[2]))
      .add_option(
          Value<unsigned>("--sbin_height", &binSize[2 + kY], "sbin height (#rows) in vertical direction").default_value(defaultParam.binSize[3]))
      .add_option(Value<double>("--bin_space_threshold",
                                &binSpaceThreshold,
                                "when the capacity of a bin (exclude fixed macros) is smaller than a specific percentage of the bin area, do not "
                                "take into the calculation for abu density")
                      .default_value(defaultParam.binSpaceThreshold))
      .add_option(Value<std::vector<std::string>>("--abu", &vAbuPercStr, "top percentage and weight pairs of bins for abu calculation")
                      .default_value(std::vector<std::string>(defaultAbuPercStr, defaultAbuPercStr + 4), "2,10 5,5 10,2 20,1"))
      .add_option(Value<std::set<std::string>>("--def_ignore_cells", &sDefIgnoreCellType, "cells ignored in input DEF file"))
      .add_option(Value<std::set<std::string>>(
          "--macro_obs_aware_layers", &sMacroObsAwareLayer, "layers of macro obstruction that are considered during placement"))
      ...
      .add_option(
          Value<bool>("--evaluate_overlap", &evaluateOverlap, "evaluate overlapping pairs of cells").default_value(defaultParam.evaluateOverlap))
      .add_option(
          Value<bool>("--move_multi_row_cell", &moveMultiRowCell, "enable multi-row cell movement").default_value(defaultParam.moveMultiRowCell))
      .add_option(Value<bool>("--align_power_line", &alignPowerLine, "enable power line alignment for multi-row cell")
                      .default_value(defaultParam.alignPowerLine))
      ...
      .add_option(
          Value<std::string>("--file_format", &fileFormatStr, "file format to write placement solution <DEF | DEFSIMPLE | BOOKSHELF | BOOKSHELFALL>")
              .default_value(toString(defaultParam.fileFormat)))
  ...
  try {
    ...
    // set yaml flag
    if (desc.count("--yaml_input") && !yamlInput.empty()) {
      isYamlInput = true;
    }

    if (!desc.count("--bookshelf_aux_input") && !isYamlInput)  // if specified Bookshelf or YAML input, LEF/DEF input is no longer required
    {
      // if not specified, must provide LEF/DEF
      dreamplaceAssertMsg(desc.count("--lef_input") && desc.count("--def_input"), "need either Bookshelf, YAML, or LEF/DEF input files");
    }
    if (!desc.count("--def_output")) {
      if (!isYamlInput) {
        // set default value
        outputDesign = limbo::trim_file_suffix(limbo::get_file_name(defInput)) + "-out.def";
      } else {
        outputDesign = limbo::trim_file_suffix(limbo::get_file_name(yamlInput)) + "-out.yaml";
      }
    }
    ...
    std::sort(vAbuPerc.begin(), vAbuPerc.end(), boost::bind(&std::pair<int, int>::first, _1) < boost::bind(&std::pair<int, int>::first, _2));
...
void UserParam::printParams() const {
  ...
  dreamplacePrint(kINFO, "yaml_input = %s\n", yamlInput.c_str());
