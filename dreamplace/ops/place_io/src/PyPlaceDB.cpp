...
bool readYaml(PlaceDB& db) {
  ...
  if (!yamlInput.empty()) {
    ...
    dreamplacePrint(kWARN, "no YAML file specified\n");

  return true;
}

void PyPlaceDB::set(PlaceDB const& db) {
  is_yaml_input = db.isYamlDesign();
  ...
  // };
  //  general add a node
  auto addNode = [&](Node const& node, std::string const& name, Orient orient, Box<PlaceDB::coordinate_type> const& box, bool /*dist2map*/) {
  ...
  for (unsigned int i = 0; i < db.nodes().size(); ++i) {
    ...
    // else if (macro.className() != "DREAMPlace.PlaceBlockage") // fixed cells are special cases, skip placement blockages (looks like ISPD2015
    // benchmarks do not process placement blockages)
  ...
  dreamplacePrint(kDEBUG,
                  "num_terminals %d, numFixed %u, numPlaceBlockages %u, num_terminal_NIs %d\n",
                  num_terminals,
                  db.numFixed(),
                  db.numPlaceBlockages(),
                  num_terminal_NIs);
  ...
  // initialize per-node port counts by physical side (left/right/lower/upper)
  std::vector<int> vNodeNumPortsLeft(mNode2NewNodes.size(), 0);
  std::vector<int> vNodeNumPortsRight(mNode2NewNodes.size(), 0);
  std::vector<int> vNodeNumPortsLower(mNode2NewNodes.size(), 0);
  std::vector<int> vNodeNumPortsUpper(mNode2NewNodes.size(), 0);
  ...
  for (unsigned int i = 0, ie = db.pins().size(); i < ie; ++i) {
    ...
    // the rotated node size, so the side reflects the actual placed orientation.
    {
      ...
      int side                      = 0;  // 0=left, 1=right, 2=lower, 3=upper
      if (dRight < dMin) {
        dMin = dRight;
        side = 1;
      }
      ...
      if (side == 0) {
        vNodeNumPortsLeft[node.id()] += 1;
        pin_side.append(static_cast<int>(PinSideEnum::LEFT));
      } else if (side == 1) {
        vNodeNumPortsRight[node.id()] += 1;
        pin_side.append(static_cast<int>(PinSideEnum::RIGHT));
      } else if (side == 2) {
        vNodeNumPortsLower[node.id()] += 1;
        pin_side.append(static_cast<int>(PinSideEnum::LOWER));
      } else {
        vNodeNumPortsUpper[node.id()] += 1;
        pin_side.append(static_cast<int>(PinSideEnum::UPPER));
      }
    }
  ...
  for (unsigned int i = 0; i < mNode2NewNodes.size(); ++i) {
    for (unsigned int j = 0; j < mNode2NewNodes.at(i).size(); ++j) {
      if (j == 0) {
        node_num_ports_left.append(vNodeNumPortsLeft[i]);
        node_num_ports_right.append(vNodeNumPortsRight[i]);
        node_num_ports_lower.append(vNodeNumPortsLower[i]);
        node_num_ports_upper.append(vNodeNumPortsUpper[i]);
      } else {
        node_num_ports_left.append(0);
        node_num_ports_right.append(0);
        node_num_ports_lower.append(0);
        node_num_ports_upper.append(0);
      }
    }
  }
              ...
              for (PlaceDB::index_type h = grid_index_yl; h < grid_index_yh; ++h) {
                ...
                if (layer == 2) {
                  ...
                  dreamplaceAssertMsg(initial_vertical_routing_map[index] == 0,
                                      ...
                                      intersect_ratio,
                                      index,
                                      initial_vertical_routing_map[index],
...
std::pair<PyPlaceDB::coordinate_type, PyPlaceDB::coordinate_type> PyPlaceDB::getRotatedSizes(int32_t rot_degree,
                                                                                             PyPlaceDB::coordinate_type src_width,
                                                                                             PyPlaceDB::coordinate_type src_height) const {
...
std::pair<PyPlaceDB::coordinate_type, PyPlaceDB::coordinate_type> PyPlaceDB::getRotatedPinOffsets(int32_t rot_degree,
                                                                                                  PyPlaceDB::coordinate_type src_width,
                                                                                                  PyPlaceDB::coordinate_type src_height,
                                                                                                  PyPlaceDB::coordinate_type src_pin_offset_x,
                                                                                                  PyPlaceDB::coordinate_type src_pin_offset_y) const {
...
std::pair<PyPlaceDB::coordinate_type, PyPlaceDB::coordinate_type> PyPlaceDB::getFlipYPinOffsets(PyPlaceDB::coordinate_type src_width,
                                                                                                PyPlaceDB::coordinate_type src_height,
                                                                                                PyPlaceDB::coordinate_type src_pin_offset_x,
                                                                                                PyPlaceDB::coordinate_type src_pin_offset_y) const {
...
void PyPlaceDB::convertOrient() {
  ...
  for (unsigned int node_id = num_nodes - num_terminals - num_terminal_NIs; node_id < node_orient.size(); ++node_id) {
    ...
    if (src_orient != OrientEnum::N && src_orient != OrientEnum::UNKNOWN) {
      ...
      std::pair<coordinate_type, coordinate_type> sizes =
          getRotatedSizes(rot_degree, node_size_x[node_id].cast<coordinate_type>(), node_size_y[node_id].cast<coordinate_type>());
      ...
      for (unsigned int j = 0; j < pins.size(); ++j) {
        ...
        std::pair<coordinate_type, coordinate_type> pin_offsets = getRotatedPinOffsets(rot_degree,
                                                                                       node_size_x[node_id].cast<coordinate_type>(),
                                                                                       node_size_y[node_id].cast<coordinate_type>(),
                                                                                       pin_offset_x[pin_id].cast<coordinate_type>(),
                                                                                       pin_offset_y[pin_id].cast<coordinate_type>());
      ...
      if (flip) {
        ...
        for (unsigned int j = 0; j < pins.size(); ++j) {
          ...
          // apply rotations to get new pin offsets
          std::pair<coordinate_type, coordinate_type> pin_offsets = getFlipYPinOffsets(node_size_x[node_id].cast<coordinate_type>(),
                                                                                       node_size_y[node_id].cast<coordinate_type>(),
                                                                                       pin_offset_x[pin_id].cast<coordinate_type>(),
                                                                                       pin_offset_y[pin_id].cast<coordinate_type>());
...
void PyPlaceDB::computeAreaStatistics() {
  ...
  for (index_type i = num_nodes - num_terminals - num_terminal_NIs; i < num_nodes - num_terminal_NIs; ++i) {
    ...
    fixed_boxes.emplace_back(node_x[i].cast<coordinate_type>(),
                             node_y[i].cast<coordinate_type>(),
                             node_x[i].cast<coordinate_type>() + node_size_x[i].cast<coordinate_type>(),
                             node_y[i].cast<coordinate_type>() + node_size_y[i].cast<coordinate_type>());
  }
